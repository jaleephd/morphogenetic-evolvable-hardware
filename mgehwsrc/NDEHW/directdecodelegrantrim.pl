...
# directdecodelegransynch.pl by J.A. Lee, April 2004
#
# Synopsis:
# 	[perl] directdecodelegransynch.pl [ -d ] outfilename outfext
#
#  Args:
...
#
#  Description:
# 		directdecodelegransynch takes chromosomes (from stdin),
...
#		input lines, LUT fn, out bus mux settings & out bus to single
# 		mux settings, in that order. These take up 17, 16, 2, 12 bits,
# 		respectively. Hence the chromosome is divided into 47 bit
# 		sections, one for each logic element, with excess bits
# 		discarded.


$LECODELEN=47;  # require 47 bits to encode a LE: 17in+16fn,2bus,12out
...
# for each cell the settings bits will look like: iiiiIIffffbboooooooo
# decoded as:
...
sub decodeBits {
  ...
  $input2=decode_input(substr($bits,8,4),3);
  $input2=decode_input(substr($bits,12,5),4);
  $lut=decode_lut(substr($bits,17,16));
  $bus=decode_bus(substr($bits,33,2));
  $output=decode_output(substr($bits,35,12));

  $decodedLE="+,+,+:" . join(";",$input1,$input2,$lut,$bus,$output);
}
